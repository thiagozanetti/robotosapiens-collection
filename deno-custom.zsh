...
'(--lock --lock-write)--no-lock[Disable auto discovery of the lock file.]' \
...
'--inspect-brk=[Activate inspector on host:port, wait for debugger to connect and break at the start of user script]:HOST:PORT: ' \
'--inspect-wait=[Activate inspector on host:port and wait for debugger to connect before running user code]:HOST:PORT: ' \
'--location=[Value of '\''globalThis.location'\'' used by some web APIs]:HREF:_urls' \
'--v8-flags=[Set V8 command line options]: : ' \
'--seed=[Set the random number generator seed]:NUMBER: ' \
...
'--v8-flags=[Set V8 command line options]: : ' \
'--seed=[Set the random number generator seed]:NUMBER: ' \
'(--inspect --inspect-wait --inspect-brk)--watch=[Watch for file changes and restart automatically]:FILES:_files' \
'-L+[Set log level]: :(debug info)' \
'--log-level=[Set log level]: :(debug info)' \
'--no-remote[Do not resolve remote modules]' \
'--no-npm[Do not resolve npm modules]' \
...
'(--inspect --inspect-wait --inspect-brk)--coverage=[UNSTABLE: Collect coverage profile data into DIR]:DIR: ' \
